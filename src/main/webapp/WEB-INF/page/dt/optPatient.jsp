	...
	<div class="easyui-layout" style="width: 1100px; height: 520px;">
		<div region="west" split="true" title="我的病人" style="width: 150px;">
			<c:forEach items="${diagList }" var="reg" varStatus="status">
				<li class="menu_li"><c:choose>
						<c:when test="${reg.flag==1}">
							<a style="color: #008B8B;"
								onclick="check(${status.index},'${reg.patientInfo.patientName}');"
								id="id_${status.index }"
								href="<%=path %>/dt/patientPerscription?registerId=${reg.registerId }"
								target="diagFrame">${reg.patientInfo.patientName }(已就诊)</a>
						</c:when>
						<c:otherwise>
							<a style="color: #CD5C5C;"
								onclick="check(${status.index},'${reg.patientInfo.patientName}');"
								id="id_${status.index }"
								href="<%=path %>/dt/dtInfoAdd?registerId=${reg.registerId }"
								target="diagFrame">${reg.patientInfo.patientName }</a>
						</c:otherwise>
					</c:choose></li>
			</c:forEach>
		...
		<div id="content" region="center" title="就诊" style="padding: 5px;">
			...
			<div region="center"
				style="height: 470px; width: 950px; background: #B3DFDA; padding: 10px">
				<iframe name="diagFrame"
					style="width: 950px; min-height: 100%; border: none;" src=""></iframe>
			</div>
