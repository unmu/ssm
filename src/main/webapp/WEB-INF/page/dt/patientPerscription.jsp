...
<script type="text/javascript">
        ...
        function formatterItemBtn(value, row, index) {
            var str = '<a href="#" name="editbtn" class="easyui-linkbutton" style="padding:2px 10px;background: #006699;color: #fefefe" title="修改药品信息" onclick="editMp(&quot;' + row.perscriptionId + '&quot;,' + index + ');">修改</a> &nbsp;&nbsp;'
                + '<a href="#" name="removebtn" class="easyui-linkbutton" style="padding:2px 10px;background: #006699;color: #fefefe" title="删除药品信息"  onclick="deleteMp(&quot;' + row.perscriptionId + '&quot;,' + index + ');" >删除</a>';
            return str;

        }

        <%-- function searchMedicine() {
            var medicineName = $("#searchMedicineName").textbox("getValue");
            $.post("<%=path%>/dt/searchMedicineInfoList", { medicineName: medicineName }, function (data, textStatus) {
                if (textStatus == "success") {
                    $('#dg_medicine').datagrid('loadData', { total: data.length, rows: data });
                }
            })
        } --%>
    ...
    <!--项目表格编辑 -->
        ...
        function searchItem() {
            ...
            $.post("<%=path%>/dt/searchItemInfoList", { itemName: itemName }, function (data, textStatus) {
                if (textStatus == "success") {
    ...
    <%-- <% --保存检查项目 --%>
        ...
        function formatterItemBtn(value, row, index) {
            debugger
            var str = '<a href="#" name="editbtn" class="easyui-linkbutton" style="padding:2px 10px;background: #006699;color: #fefefe" title="修改检查信息" onclick="editChk(&quot;' + row.itemId + '&quot;,' + index + ');">修改</a> &nbsp;&nbsp;'
                + '<a href="#" name="removebtn" class="easyui-linkbutton" style="padding:2px 10px;background: #006699;color: #fefefe" title="删除检查信息"  onclick="deleteChk(&quot;' + row.itemId + '&quot;,' + index + ');" >删除</a>';
            return str;
        ...
        function deleteChk(itemId, index) {
            console.log(itemId)
...
<body style="padding: 0px">
	<div class="easyui-tabs" style="width: 98%; min-height: 99%;">
			...
			<table id="dg_chk" class="easyui-datagrid" style="width: 99%;"
				...
				<thead>
					...
					<tr>
						...
						<th
							data-options="field:'amount',width:'13%',align:'center',editor:{type:'numberspinner',options:{min:1,increment:1}}">数量</th>
						<th data-options="field:'unit',width:'8%',align:'center'">单位</th>
						<th data-options="field:'money',width:'10%',align:'center'">金额</th>
						<th
							data-options="field:'remark',width:'15%',editor:{type:'textbox',options:{multiline:true}}">医嘱</th>
						<th
							data-options="field:'action',width:'20%',formatter:formatterItemBtn,align:'center'">操作</th>
					</tr>
				</thead>
				<tbody>
		...
		<!-- 处方列表 -->
		<div title="处方列表" style="padding: 10px">处方列表</div>
